...
- 寄存器
    ...
    - `"<n>P` 复制寄存器到光标前
- 查找
    - `? <pattern>`   进入末行模式，向文件头方向查找匹配的文本
    - `/ <pattern>`   进入末行模式，向文件尾方向查找匹配的文本
    - `n`   搜索下一个匹配的字符串（配合末行模式下查找）
    - `N`   逆向搜索下一个匹配的字符串（配合末行模式下查找）
    - `*` 向前搜索当前单词
    - `#` 向后搜索当前单词
- 其他
    - `Ctrl+g`   在状态行上显示状态信息
...
- 保存与退出
    - `q`   退出。若文件没有保存，给出警告
    - `q!`   不保存而直接退出
    - `close` 退出。若文件没有保存，给出警告
    - `w`   保存
    - `w <file>`   保存，如果<file>已存在，给出警告
    - `w! <file>`   保存，如果<file>已存在，则覆盖
    - `wq`   保存并退出
    - `x`   保存并退出
    - `saveas <file>` 另存为
- 打开
