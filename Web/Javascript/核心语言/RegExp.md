...
```
Array RegExp.prototype.exec(string input)
```

对字符串进行一次匹配。在使用g或s修饰符时，正则表达式是有状态的，上次成功匹配后的位置记录在lastIndex属性中，从而可以对一个字符串进行多次匹配。

返回一个匹配数组，字段含义如下：
- `[0]` 匹配到的字符串。
- `[n]` 捕获组捕获的字符串。
- index 匹配到的字符串在原始字符串中的位置
- input 原始字符串
...
```
boolean RegExp.prototype.test(string input)
```